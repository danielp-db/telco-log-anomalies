...
"""
BPM Performance Logs Streaming Ingestion
Continuously reads JSON BPM performance logs from Volume and writes to Delta table using Structured Streaming.
"""
...
config = {
    ...
    "bpm_perf_checkpoint": f"/Volumes/{catalog}/{schema}/{volume_name}/_checkpoints/bpm_perf",
...
print(f"  Source: {config['bpm_perf_logs_path']}")
print(f"  Target: {config['bpm_perf_table']}")
print(f"  Checkpoint: {config['bpm_perf_checkpoint']}")
...
# Define the schema for BPM performance logs (same as BPM adapter)
bpm_perf_schema = StructType([
...
bpm_perf_stream = (spark
    .readStream
    .format("json")
    .schema(bpm_perf_schema)
    .option("maxFilesPerTrigger", config["max_files_per_trigger"])
    .load(config["bpm_perf_logs_path"])
)
...
bpm_perf_processed = (bpm_perf_stream
...
query = (bpm_perf_processed
    .writeStream
    .format("delta")
    .outputMode("append")
    .option("checkpointLocation", config["bpm_perf_checkpoint"])
    .trigger(processingTime=config["trigger_processing_time"])
    .option("mergeSchema", "true")
    .toTable(config["bpm_perf_table"])
)
...
print(f"\nStreaming BPM performance logs from {config['bpm_perf_logs_path']}")
print(f"Writing to table: {config['bpm_perf_table']}")
print(f"Checkpoint location: {config['bpm_perf_checkpoint']}")
