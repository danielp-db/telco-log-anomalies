...
"""
M2E Performance Logs Streaming Ingestion
Continuously reads JSON M2E performance logs from Volume and writes to Delta table using Structured Streaming.
"""
...
config = {
    ...
    "m2e_perf_checkpoint": f"/Volumes/{catalog}/{schema}/{volume_name}/_checkpoints/m2e_perf",
...
print(f"  Source: {config['m2e_perf_logs_path']}")
print(f"  Target: {config['m2e_perf_table']}")
print(f"  Checkpoint: {config['m2e_perf_checkpoint']}")
...
# Define the schema for M2E performance logs
m2e_perf_schema = StructType([
...
m2e_perf_stream = (spark
    .readStream
    .format("json")
    .schema(m2e_perf_schema)
    .option("maxFilesPerTrigger", config["max_files_per_trigger"])
    .load(config["m2e_perf_logs_path"])
)
...
m2e_perf_processed = (m2e_perf_stream
...
query = (m2e_perf_processed
    .writeStream
    .format("delta")
    .outputMode("append")
    .option("checkpointLocation", config["m2e_perf_checkpoint"])
    .trigger(processingTime=config["trigger_processing_time"])
    .option("mergeSchema", "true")
    .toTable(config["m2e_perf_table"])
)
...
print(f"\nStreaming M2E performance logs from {config['m2e_perf_logs_path']}")
print(f"Writing to table: {config['m2e_perf_table']}")
print(f"Checkpoint location: {config['m2e_perf_checkpoint']}")
